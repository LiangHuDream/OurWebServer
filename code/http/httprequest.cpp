...
bool HttpRequest::ParseRequestLine_(const string& line) {
    // Define the regular expression pattern for the request line.
    regex pattern("^([^ ]*) ([^ ]*) HTTP/([^ ]*)$");

    // Create a match object to store the submatches.
    smatch subMatch;

    // Check if the line matches the pattern.
    if(regex_match(line, subMatch, pattern)) {   
        // Extract the method, path, and version from the submatches.
        method_ = subMatch[1];
        path_ = subMatch[2];
        version_ = subMatch[3];

        // Update the state to HEADERS.
        state_ = HEADERS;
    ...
    // Log an error and return false if the parsing failed.
...
// Get the version of the HTTP request
std::string HttpRequest::version() const {
    return version_;
}

// Get the value of a POST parameter with the given key
std::string HttpRequest::GetPost(const std::string& key) const {
    ...
    // Return an empty string if the POST parameter does not exist
    return "";
}

/**
 * @brief Get the value of a specific key from the POST data.
 * 
 * @param key The key to search for in the POST data.
